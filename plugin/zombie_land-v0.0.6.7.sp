...
public Action OnPlayerChangeClass(Handle event, const char[] name, bool dontBroadcast)
{
	if(b_isZLmap)
	{
		...
		{
			#if DEBUG
				PrintToServer("[OnPlayerChangeClass] Setting Engineer class on (%N) - Red Team", client);
			#endif
		...
		{
			#if DEBUG
				PrintToServer("[OnPlayerChangeClass] Setting Medic class & force MeleeOnly on (%N) - Blue Team", client);
			#endif
...
public Action TF2_PlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if(b_isZLmap && (team == TFTeam_Red || team == TFTeam_Blue))
	{
		if(b_GameStarted)
		{
			...
			#if DEBUG
				PrintToServer("[OnPlayerSpawn] iBlues Value: %i", iBlues);
			#endif
...
public Action TF2_PlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if(b_GameStarted && (team == TFTeam_Red || team == TFTeam_Blue))
	{
		...
		else 
		{
			...
			#if DEBUG
				PrintToServer("[OnPlayerDeath - GameStarted] Client (%N) is in blue team. iBlues: %i", client, iBlues);
			#endif
		}
...
public Action StartZombieMod(Handle timer)
{
	...
	#if DEBUG
		PrintToServer("[StartZombieMod - Timer_CallBack] iBlues Value: %i - Making doors disable...", iBlues);
	#endif
