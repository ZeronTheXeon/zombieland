	...
	if (b_IsZlMap)
	{
		...
		if (b_GameStarted)
		{
			if (team == TFTeam_Red)
			{
				...
				PrintCenterText(client, "[Zombie-Land]: %T", "Spawn_Red_Block");
		...
		else
		{
			if (team == TFTeam_Blue)
			{
				...
				PrintCenterText(client, "[Zombie-Land]: %T", " Spawn_Blu_Block");
...
public Action StartZombieMod(Handle timer)
{
	...
	PrintToChatAll("[Zombie-Land]: %T", " Zomb_Release_Msg");
}
