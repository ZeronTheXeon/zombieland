...
public void OnPluginStart()
{
	...
	h_MinPlayers = CreateConVar("sm_zl_minplayers", "2", "Min players to start the game", _, true, 1.0);
...
public Action OnPlayerInventory(Handle event, const char[] name, bool dontBroadcast)
{
	if(b_IsZlMap && IsPlayersEnough())
	{
		...
		{
			#if DEBUG
				PrintToServer("[OnPlayerInventory] Setting MeleeOnly on client: %N", client);
			#endif
			
			TF2_RemoveWeaponSlot(client, TFWeaponSlot_Primary);
			TF2_RemoveWeaponSlot(client, TFWeaponSlot_Secondary);
			
			TF2_RemoveWearable(client, 198);
			TF2_RemoveWearable(client, 37);
			TF2_RemoveWearable(client, 173);
			TF2_RemoveWearable(client, 264);
			TF2_RemoveWearable(client, 304);
			TF2_RemoveWearable(client, 413);
			TF2_RemoveWearable(client, 423);
			TF2_RemoveWearable(client, 474);
			TF2_RemoveWearable(client, 880);
			TF2_RemoveWearable(client, 939);
			TF2_RemoveWearable(client, 954);
			TF2_RemoveWearable(client, 1003);
			TF2_RemoveWearable(client, 1013);
			TF2_RemoveWearable(client, 1071);
			TF2_RemoveWearable(client, 1123);
			TF2_RemoveWearable(client, 1127);
			TF2_RemoveWearable(client, 1143);
			TF2_RemoveWearable(client, 30758);
...
public Action TF2_PlayerSpawn(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if(b_IsZlMap && (team == TFTeam_Red || team == TFTeam_Blue))
	{
		if(b_GameStarted)
		{
			...
			{
				#if DEBUG
					PrintToServer("[OnPlayerSpawn - GameStarted] Changing client(%N) team from red to blue", client);
				#endif
			...
			{
				#if DEBUG
					PrintToServer("[OnPlayerSpawn] Restaring round because all players are in blue team.");
				#endif
		...
		else
		{
			...
			{
				#if DEBUG
					PrintToServer("[OnPlayerSpawn - !GameStarted] Changing client(%N) team from blue to red", client);
				#endif
				
				TF2_ChangeClientTeam(client, TFTeam_Red);
				PrintCenterText(client, "[Zombie-Land] You can't spawn on BLU. Switching to RED.");
				SetPlayerClass(client, TFTeam_Red);
				return Plugin_Changed;
			}
		}
	}
...
public Action TF2_PlayerDeath(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if(b_GameStarted && (team == TFTeam_Red || team == TFTeam_Blue))
	{
		...
		{
			#if DEBUG
				PrintToServer("[OnPlayerDeath - GameStarted] Changing client(%N) team from red to blue", client);
			#endif
		...
		#if DEBUG
			PrintToServer("[OnPlayerDeath - GameStarted] Client (%N) has respawned.", client);
		#endif
...
public Action TF2_RoundStart(Handle event, const char[] name, bool dontBroadcast)
{
	...
	{
		#if DEBUG
			PrintToServer("[RoundStart - IsZLMap] Timers created.");
		#endif
...
public Action TF2_RoundEnd(Handle event, const char[] name, bool dontBroadcast)
{
	...
	{
		#if DEBUG
			PrintToServer("[RoundEnd - GameStarted] Setting game started to false & enabling doors.");
		#endif
...
{
	#if DEBUG
		PrintToServer("[StartZombieMod - Timer_CallBack] Moving players , SetClasses and start the game.");
	#endif
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i))
		{
			...
			{
				#if DEBUG
					PrintToServer("[StartZombieMod - Timer_CallBack] Moving all players to red. client: %N", i);
				#endif
	...
	#if DEBUG
		PrintToServer("[StartZombieMod - Timer_CallBack] Randomizing blue players of 20 precent value is: %i", Blue_Count);
	#endif
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		{
			#if DEBUG
				PrintToServer("[StartZombieMod - Timer_CallBack] Moving random players to team blue. client: %N", i);
			#endif
	...
	#if DEBUG
		PrintToServer("[StartZombieMod - Timer_CallBack] Making doors disable...");
	#endif
...
public void SetPlayerClass(int client, TFTeam team)
{
	...
	{
		#if DEBUG
			PrintToServer("[SetPlayerClass] Red client (%N) detected. Setting class to Engineer...", client);
		#endif
	...
	{
		#if DEBUG
			PrintToServer("[SetPlayerClass] Blue client (%N) detected. Setting class to Medic & Setting MeleeOnly...", client);
		#endif
...
{
	#if DEBUG
		PrintToServer("[ToggleObjectiveState] Toggling objective state (%s)", newState ? "True":"False");
	#endif
...
{
	#if DEBUG
		PrintToServer("[SwitchToSlot] Switching to MeleeOnly Slot Client (%N)", client);
	#endif
