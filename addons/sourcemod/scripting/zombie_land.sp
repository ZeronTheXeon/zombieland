	...
	if (b_IsZlMap)
	{
		...
		if (b_GameStarted)
		{
			if (team == TFTeam_Red)
			{
				...
				PrintCenterText(client, "{GREEN}[Zombie-Land]{DEFAULT}: %T", "Spawn_Red_Block");
		...
		else
		{
			if (team == TFTeam_Blue)
			{
				...
				PrintCenterText(client, "{GREEN}[Zombie-Land]{DEFAULT}: %T", " Spawn_Blu_Block");
...
public Action StartZombieMod(Handle timer)
{
	...
	PrintHintTextToAll("%T", "Zomb_HintRelease_Msg");
	PrintToChatAll("{LIME}[Zombie-Land]{DEFAULT}: %T", " Zomb_Release_Msg");
}
...
public Action CountDown(Handle timer)
{
	...
	PrintHintTextToAll("%T", "ZLstart_Hint_Msg");
